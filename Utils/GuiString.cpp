...
namespace Logic
{
   ...
   GuiString  GuiString::ReplaceAll(const wstring& substring, const wstring& replacement) const
   {
      GuiString s(*this);

      // Store invariants
      LPCWSTR search = substring.c_str();
      size_t  search_len = substring.length(),
              replace_len = replacement.length();
